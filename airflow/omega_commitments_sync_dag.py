...
SCHEDULE = os.getenv("OMEGA_COMMITMENTS_SYNC_SCHEDULE", "*/2 6-23 * * *")
...
def _run_sync() -> None:
    script_win = str(Path(APP_WIN) / "tool" / "commitments_sync.py")
    ...
    if result.returncode != 0:
        raise AirflowException(
            "Falha ao sincronizar filial.tauxcompromisso com o aplicativo "
            f"(exit code {result.returncode})"
        )


with DAG(
    dag_id="omega_commitments_sync",
    description=(
        "Sincroniza compromissos comerciais somente quando a fonte local muda."
    ),
    default_args={
        "owner": "omega",
    ...
    start_date=pendulum.datetime(2026, 8, 21, 6, 0, tz=LOCAL_TZ),
    schedule=SCHEDULE,
    catchup=False,
    max_active_runs=1,
    tags=["omega", "postgres", "supabase", "compromissos"],
) as dag:
    sync_commitments = PythonOperator(
        task_id="sync_commitments_if_changed",
