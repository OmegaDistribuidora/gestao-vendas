...
begin
  ...
  available_supplier_rows as (
    select
      s.codfornec,
      coalesce(sp.supplier_name, s.codfornec) as supplier_name,
      row_number() over (
        partition by s.codfornec
        order by coalesce(sp.supplier_name, s.codfornec), s.codfornec
      ) as rn
    from public.app_sales_daily_snapshots s
    join scoped_base sb
      on sb.codcli = s.codcli
     and sb.codusur = s.codusur
    left join public.app_suppliers sp on sp.codfornec = s.codfornec
    where s.sales_date >= (local_today - interval '180 days')::date
  ),
  base_rows as (
    select sb.*
    from scoped_base sb
  ...
  select jsonb_build_object(
    ...
    'available_suppliers', coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'code', supplier.codfornec,
          'supplier_name', supplier.supplier_name
        )
        order by supplier.supplier_name, supplier.codfornec
      )
      from available_supplier_rows supplier
      where supplier.rn = 1
    ), '[]'::jsonb),
