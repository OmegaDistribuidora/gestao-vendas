create table if not exists public.app_delinquency_items (
  id uuid primary key default gen_random_uuid(),
  dtemissao date not null,
  dtpag date,
  dtvenc date not null,
  numped text not null,
  numtransvenda text not null default '',
  ...
  prestacao text not null default '',
  duplicata text not null default '',
  codcob text not null default '',
  tipo text not null default 'Geral',
  codcoborig text not null default '',
  codfilial text not null default '',
  status text not null default '',
  valor numeric(18, 2) not null default 0,
  vpago numeric(18, 2) not null default 0,
  valordesc numeric(18, 2) not null default 0,
  valororig numeric(18, 2) not null default 0,
  imported_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  unique (dtvenc, numped, codcli, codusur, prestacao, duplicata)
);

create index if not exists idx_app_delinquency_items_due_user
  on public.app_delinquency_items (dtvenc desc, codusur);

create index if not exists idx_app_delinquency_items_due_supervisor
  on public.app_delinquency_items (dtvenc desc, codsupervisor);

create index if not exists idx_app_delinquency_items_due_manager
  on public.app_delinquency_items (dtvenc desc, codgerente);

create index if not exists idx_app_delinquency_items_client
  on public.app_delinquency_items (codcli, dtvenc desc);

create index if not exists idx_app_delinquency_items_order
  on public.app_delinquency_items (numped, dtvenc desc);

drop trigger if exists set_app_delinquency_items_updated_at on public.app_delinquency_items;
create trigger set_app_delinquency_items_updated_at
before update on public.app_delinquency_items
for each row
execute function public.set_updated_at();

alter table public.app_delinquency_items enable row level security;

grant select, insert, update, delete on public.app_delinquency_items to authenticated, service_role;

drop policy if exists "delinquency_items_admin_manage" on public.app_delinquency_items;
create policy "delinquency_items_admin_manage"
on public.app_delinquency_items
for all
to authenticated
using (public.is_admin())
with check (public.is_admin());

drop function if exists public.get_delinquency_overview(text, text);
