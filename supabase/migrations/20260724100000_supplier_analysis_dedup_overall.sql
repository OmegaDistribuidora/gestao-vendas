    ...
    select jsonb_build_object(
      ...
      'overall', (
        select jsonb_build_object(
          'code', '__geral__',
          'supplier_name', 'Geral',
          'gross_amount', coalesce(og.gross_amount, 0),
          'return_amount', coalesce(orows.return_amount, 0),
          'gross_volume', coalesce(og.gross_volume, 0),
          'return_volume', coalesce(orows.return_volume, 0),
          'gross_orders', coalesce(og.gross_orders, 0),
          'return_orders', coalesce(orows.return_orders, 0),
          'gross_positivation', coalesce(og.gross_positivation, 0),
          'return_positivation', coalesce(orows.return_positivation, 0)
        )
        from overall_gross og
        cross join overall_returns orows
      ),
      'suppliers',
      coalesce((
        select jsonb_agg(
