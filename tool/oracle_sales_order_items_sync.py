...
SELECT
  ...
  END AS codfornec,
  pcpedi.codprod,
  INITCAP(pcprodut.descricao) AS produto,
  SUM(pcpedi.qt * pcpedi.pvenda) AS valor,
...
def fetch_oracle_rows(sync_start_date: date) -> list[SalesOrderItemRow]:
...
def stage_sales_order_item_rows(
    session: SupabaseSession,
    run_id: str,
    rows: Iterable[SalesOrderItemRow],
) -> int:
    deduped_rows = {
        (
            row.sales_date,
    ...
    return insert_rows(session, "etl_stg_sales_order_items", payload)


def apply_sales_order_items_sync(
    session: SupabaseSession,
    run_id: str,
) -> dict[str, object]:
    response = invoke_rpc(
        session,
        "apply_sales_order_items_sync",
        {"p_run_id": run_id},
    )
...
def main() -> None:
    ...
    run_id = begin_sync_run(
        session,
        job_name="oracle_sales_order_items_sync",
        target_name="app_sales_order_items",
    ...
    try:
        staged_count = stage_sales_order_item_rows(session, run_id, rows)
        set_sync_run_rows_staged(session, run_id, staged_count)
        apply_result = apply_sales_order_items_sync(session, run_id)
